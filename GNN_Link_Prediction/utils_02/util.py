...
MODEL_MODE = Literal[1, 2, 3]


def save_result(result, root_path, mode: MODEL_MODE):
    """
    保存模型在验证集上的训练结果

    :param result: 训练数据结果
    :param root_path: 需要保存的根目录
    :param mode: 模式，也就是步长，1，2，3
    :return: none
    """
    model_path = join(root_path, 'model_' + str(mode))
    if not os.path.exists(model_path):
        os.makedirs(model_path)
    result_path = join(model_path, 'gnn_result.tsv')
...
def save_model(model, root_path, mode: MODEL_MODE):
    """
    保存训练好的模型
    ...
    if not os.path.exists(model_path):
        os.makedirs(model_path)
    model_path = join(model_path, 'gnn_model.pth')
    torch.save(model.state_dict(), model_path)
    print(f'Model saved to {model_path}')


def load_model(model, root_path, mode: MODEL_MODE):
    """
    加载训练好的模型 \n
