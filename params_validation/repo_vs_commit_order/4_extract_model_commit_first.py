...
def main_func(git_path: list[str], project_name: str, project_input: str):
    commit_index_threshold = 1  # 最近的commit_index的阈值 待取值为1，2，3,都要跑一次
    ...
    for index_dir in file_dir_list:
        ...
        for project_dir in project_list:
            ...
            for xml_file in xml_list:
                ...
                # model_exist = join(root_path, f'my_{project_name}', f'commit_first_{commit_index_threshold}', xml_file[0:xml_file.find('.')], 'code_context_model.xml')
                # if os.path.exists(model_exist):
                #     print('exist....')
                #     continue
                # 创建目录，保存文件，方便运行doxygen
                make_root_path = make_dir(join(root_path, f'my_{project_name}', f'commit_first_{commit_index_threshold}',
                ...
                for commit_index in range(commit_index_threshold):
                    ...
                    for git_repo in git_path:
                        ...
                        if len(commits) <= commit_index:  # 如果commit不足，跳过处理
                            continue
                        ...
                        print("all repo finding completed!!!")
                        continue
...
# pde
# main_func([join(root_path, 'pde', 'eclipse.pde')], 'pde', 'PDE')
# platform
# main_func([
