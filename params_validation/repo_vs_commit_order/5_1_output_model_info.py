...
def show_model(context_models: list, model_info: str):
    ...
    print("min_node:{0}, max_node:{1}, median_node:{2}, mean_node:{3}, std_node:{4}".format(min_node, max_node,
                                                                                            median_node, mean_node,
                                                                                            std_node))
    ...
    # plt.show()
    print("min_edge:{0}, max_edge:{1}, median_edge:{2}, mean_edge:{3}, std_edge:{4}".format(min_edge,
    ...
    # plt.show()
    print(
        "min_component:{0}, max_component:{1}, median_component:{2}, mean_component:{3}, std_component:{4}".format(
            min_component,
            max_component,
            median_component,
            mean_component,
...
def show_component(connected_components: list, model_info: str):
    ...
    # plt.show()
    print(
        "min_diameter:{0}, max_diameter:{1}, median_diameter:{2}, mean_diameter:{3}, std_diameter:{4}".format(
            min_diameter,
            max_diameter,
            median_diameter,
            mean_diameter,
...
def main_func(mode: str):
    info_dir_path = join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), f'{mode}model_info')
    info_strategy_dir = os.listdir(info_dir_path)
    for info_strategy in info_strategy_dir:
        info_strategy_path = join(info_dir_path, info_strategy)
        print(f'---------{info_strategy_path}----------')
        model_info_list = os.listdir(info_strategy_path)
        for model_info in model_info_list:
            print(model_info)
            model_file = join(info_strategy_path, model_info)
