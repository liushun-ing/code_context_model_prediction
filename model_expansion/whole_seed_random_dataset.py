"""
将 code context model 拆分为seed 穷举所有可能seed
"""
import os
import random
...
def extract_seed(project_model_name: str):
    ...
    for model_dir in model_dir_list:
        ...
        for step in steps:
            ...
            # 将XML写入文件
            new_tree.write(join(model_path, f'{step}_step_model_sample.xml'))


def expand_random_dataset(project_model_name: str, step: int):
    project_path = join(root_path, project_model_name, 'repo_first_3')
    model_dir_list = os.listdir(project_path)
    ...
    for model_dir in model_dir_list:
        ...
        expand_graph.expand_model(seed_graphs, all_repo_metrics, step)
        model_loader.save_expanded_model(seed_graphs, join(model_path, f'{step}_step_model_sample.xml'))
