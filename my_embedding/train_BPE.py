...
my_root = join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'my_embedding')
...
def dictionary_and_embedding(trees, size, step, ratio, description):
    ...
    Returns: 无
    """
    if not os.path.exists(join(my_root, 'tokens')):
        os.mkdir(join(my_root, 'tokens'))
    ...
    tokenizer.save(join(my_root, 'tokens', f'{description}_{str(step)}_{str(ratio)}_tokens_{str(size)}.json'))
