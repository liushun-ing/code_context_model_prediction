    ...
    for graph in graphs:
        ...
        for node in vertex_list:
            if node.get('origin') == '1':
                print(node.get('id'), node.get('origin'), node.get('label'))
            g.add_node(int(node.get('id')), label=node.get('stereotype'), origin=node.get('origin'))
        for link in edge_list:
            if link.get('origin') == '1':
                print(link.get('start'), link.get('end'), link.get('label'))
...
def load_targets(project_model_name: str, step):
    ...
    index = model_dir_list.index('5796')
    ...
    for G1 in G1s[1:2]:
        ...
        for top_c in confidence:
            if top_c[1] > MinConf:
                print(top_c[1])
                node_id = top_c[0]
                print(node_id, G1.nodes.get(node_id))
